        ...
        <motion.div className="fixed inset-0 z-[60]" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto">
              {lines.length === 0 ? (
                <p className="px-6 py-12 text-center text-sm text-muted-foreground">Your cart is empty.</p>
              ) : (
                <div className="space-y-5 px-6 py-4">
                      ...
                      <li key={l.id} className="rounded-2xl border border-border p-4">
                        <div className="flex items-start justify-between gap-3">
                          <div className="flex min-w-0 items-start gap-3">
                            {l.image_url && (
                              <img src={l.image_url} alt={l.name} className="h-14 w-14 flex-shrink-0 rounded-lg object-cover" />
                            )}
                            <div className="min-w-0">
                              <div className="truncate font-display font-semibold">{l.name}</div>
                              <div className="truncate text-xs text-muted-foreground">{l.spec} · {l.line_name}</div>
                              <div className="mt-1 font-mono text-sm text-primary">{formatNaira(l.price)}</div>
                            </div>
                          </div>
                          <button onClick={() => remove(l.id)} className="flex-shrink-0 text-muted-foreground hover:text-destructive" aria-label="Remove">
                  ...
                  <div className="space-y-3 border-t border-border pt-4">
                    <div className="grid grid-cols-2 gap-3">
                      <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Full name" className="min-w-0 rounded-full border border-border bg-background px-4 py-2.5 text-sm outline-none focus:border-primary" />
                      <input value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="Phone" className="min-w-0 rounded-full border border-border bg-background px-4 py-2.5 text-sm outline-none focus:border-primary" />
                    </div>
                    ...
                    <label className="flex cursor-pointer items-center justify-between gap-3 rounded-2xl border-2 border-dashed border-border bg-background px-4 py-3 text-sm hover:border-primary">
                      ...
                      <span className="flex-shrink-0 text-xs text-muted-foreground">{proofFile ? "Change" : "Required"}</span>
                      <input
                        type="file"
